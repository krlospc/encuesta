...
class DefaultController extends Controller
{
    ...
    public function insertAction(Request $request)
    {
        ...
        for ($i = 1; $i <= 28; $i++) {
            ...
            if (($i == 27) or ($i == 28)){
                $factor = "";
                if (isset($_REQUEST['factor_'.$i])) {
                    $factor = $_REQUEST['factor_'.$i];
                    $query = "UPDATE formulario_uno_factores SET seleccion = ?, orden_importancia = ?, por_que = ?, factor = ? where id = ?";            
                    $stmt = $db->prepare($query);
                    $params = array($sel, $orden, $porque, $factor, $idfac);
                    $stmt->execute($params);
                } else {
    ...
    public function newdisplayAction()
    {
        $arrayfactortipo = [
            "1" => "Dentro de la unidad educativa: factores atribuidos al estudiante",
            "2" => "Dentro de la unidad educativa: factores atribuidos a la unidad educativa",
            "3" => "Fuera de la unidad educativa: factores familiares",
            "4" => "Fuera de la unidad educativa: factores economicos",
            "5" => "Fuera de la unidad educativa: factores sociales",
            "6" => "Otros factores",
        ];    
        ...
        return $this->render('FormUnoBundle:Default:newdisplay.html.twig', array('arrayfactortipo' => $arrayfactortipo, 'arrayfactor' => $arrayfactor,));
    }

    public function insertdisplayAction()
    {
        $em = $this->getDoctrine()->getManager();        
        $db = $em->getConnection();
        ...
        $query = $query."VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";          
        $stmt = $db->prepare($query);
        $params = array($dep, $cod_unidad_educativa, $nboleta, $nombre, $paterno, $materno, $unidad_educativa, $sexo, $telefono, $cargo, $p8, $p9_1, $p10, $p12, $p13, $p9_2, $p9_3, $p9_4, $p9_5, $p9_6);
        $stmt->execute($params);
        ...
        for ($i = 1; $i <= 28; $i++) {
            ...
            if (($i == 27) or ($i == 28)){
                ...
                if (isset($_REQUEST['factor_'.$i])) {
                    ...
                    $query = $query."VALUES (?, ?, ?, ?, ?, ?, ?)";            
                    $stmt = $db->prepare($query);
                    $params = array($factor_tipo, $factor, $sel, $orden, $porque, $i, $id);
                    $stmt->execute($params);
                } else {
                    $query = "INSERT INTO formulario_uno_factores (factor_tipo, factor, seleccion, orden_importancia, por_que, cod, formulario_uno_index_id) ";
                    $query = $query."VALUES (?, ?, ?, ?, ?, ?, ?)";            
                    $stmt = $db->prepare($query);
                    $params = array($factor_tipo, $array[$i], $sel, $orden, $porque, $i, $id);
                    $stmt->execute($params);
                }
            } else {
                $query = "INSERT INTO formulario_uno_factores (factor_tipo, factor, seleccion, orden_importancia, por_que, cod, formulario_uno_index_id) ";
